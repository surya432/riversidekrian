<div class="btn-group">
    ...
    @endif
    @if(!empty($url_link))
    <a href="{{ $url_link }}" class="btn btn-sm btn-success btn-sm btn-flat "><i class='fa fa-eye'></i></a>
    @endif
    @if(!empty($url_view))
    ...
    @endif
    @if(!empty($url_edit))
    <button link="{{ $url_edit }}" class='btn btn-sm btn btn-primary btn-edit btn-action btn-sm btn-flat '><i class='fa fa-edit' aria-hidden="true"></i></button>
    @endif
    @if(!empty($url_hapus))
